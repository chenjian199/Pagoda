...
use pagoda_runtime::engine::{AsyncEngine, AsyncEngineContext, Data, ResponseStream};
...
pub struct MockNetworkTransport<T: PipelineIO, U: PipelineIO> {
    req: std::marker::PhantomData<fn() -> T>,
    resp: std::marker::PhantomData<fn() -> U>,
}
...
pub struct MockNetworkEgress<Req: PipelineIO, Resp: PipelineIO> {
    ...
    req: std::marker::PhantomData<fn() -> Req>,
    resp: std::marker::PhantomData<fn() -> Resp>,
}
