...
//!
//! ## 设计意图
//! 实现实时重放运行时，构建共享路由器、worker 调度器与 demux 输入，并驱动有限队列或多轮 workload 的并发重放。
//!
//! ## 外部契约
//! 提供 `LiveRuntime` 及其 `new`/运行接口，返回 `TraceSimulationReport`/统计，行为与 Dynamo 一致。
...
impl LiveRuntime {
    /// 为一次实时重放运行构建共享路由器、worker 调度器与 demux 输入。
    pub(super) fn new(
        args: MockEngineArgs,
    ...
    /// 重放一个有限请求队列，并返回最终 trace 报告与调试统计。
    ...
    /// 驱动多轮 workload 驱动器，直到其被排空且所有派生的请求任务均完成。
