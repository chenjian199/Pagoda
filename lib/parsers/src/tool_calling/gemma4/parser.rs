...
pub fn detect_tool_call_start_gemma4(chunk: &str) -> bool {
    ...
    (1..TOOL_CALL_START.len())
        .filter(|&i| TOOL_CALL_START.is_char_boundary(i))
...
pub fn try_tool_call_parse_gemma4(
    ...
    for caps in regex.captures_iter(message) {
        ...
        // 参数解析失败时退化为空对象并告警
        let args_value = parse_args_object(args_raw).unwrap_or_else(|e| {
            tracing::warn!(
...
fn parse_key(cur: &mut Cursor) -> anyhow::Result<String> {
    ...
    let start = cur.pos;
    while let Some(&b) = bytes.get(cur.pos) {
...
fn try_consume_keyword(cur: &mut Cursor, keyword: &str) -> bool {
    ...
    if end > bytes.len() || !bytes[cur.pos..end].eq_ignore_ascii_case(kw) {
        return false;
    }
    // 边界检查：后续若为单词字符则不算关键字
    if bytes
...
fn parse_value(cur: &mut Cursor) -> anyhow::Result<Value> {
    ...
    // 包裹字符串 `<|"|>...<|"|>`；闭合分隔符缺失（截断）时取起始符之后全部为值
    if let Some(stripped) = cur.rest().strip_prefix(STRING_DELIM) {
        let body_start = cur.pos + STRING_DELIM.len();
        match stripped.find(STRING_DELIM) {
            Some(end_rel) => {
...
fn parse_number(cur: &mut Cursor) -> anyhow::Result<Value> {
    ...
    while bytes.get(cur.pos).is_some_and(u8::is_ascii_digit) {
        cur.pos += 1;
    }
