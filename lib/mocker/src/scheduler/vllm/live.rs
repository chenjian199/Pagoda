...
//!
//! ## 设计意图
//! 在独立异步任务中驱动 `VllmCore`，将模拟时间映射到真实壁钟，
//! 接收请求、逐轮推进、按可见性发布 KV/FPM 事件并回吐输出信号。
//!
//! ## 外部契约
//! - `MockerMetrics`（字段 dp_rank/active_decode_blocks/total_blocks/gpu_cache_usage_perc）及 `new`；
//! - `Scheduler`（`new`/`new_with_admission`）并实现 `SchedulerHandle`。
//!
//! ## 实现要点
//! - 一个 `tokio::spawn` 循环：接收请求 → execute_pass_internal → 按可见性发布 → 精确睡眠 → 回吐；
//! - `CancelGuard` 在 Scheduler 被丢弃时取消任务；watch 通道广播最新指标。
...
pub struct MockerMetrics {
    pub dp_rank: pagoda_kv_router::protocols::DpRank,
...
impl MockerMetrics {
    pub fn new(
        dp_rank: pagoda_kv_router::protocols::DpRank,
...
impl Scheduler {
    ...
    fn new_internal(
        ...
        tokio::spawn(async move {
            ...
            // 本调度器模拟时间的壁钟起点。驱动 `engine.tick(now_ms)`，
            // 使 PS 带宽模型跨多轮按真实时间推进。
            ...
            loop {
                ...
                let _ = metrics_tx.send(MockerMetrics::new(
                    dp_rank,
                    core.kv_cache.num_active_blocks() as u64,
                    total_blocks,
                ));
            }
        });
