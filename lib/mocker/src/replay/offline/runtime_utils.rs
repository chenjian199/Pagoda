...
//!
//! ## 设计意图
//! 提供离线重放运行时共用的辅助函数，包括下一事件时间计算与事件堆的推入/出队。
//!
//! ## 外部契约
//! 提供 `next_timestamp`/`push_worker_*`/`pop_ready_worker_*` 等 pub(super) 辅助函数，行为与 Dynamo 一致。
...
mod tests {
    ...
    fn test_worker_ready_does_not_interfere_with_completion_pop() {
        ...
        // pop_ready_worker_completion 必须返回 None（事件类型不匹配）。
        assert!(pop_ready_worker_completion(&mut events, 10.0).is_none());
        // 该事件应仍在事件堆中。
        assert_eq!(events.len(), 1);
        // pop_ready_worker_ready 应该成功。
