...
//!   - [`PagodaError`] 通过 `caused_by: Option<Box<PagodaError>>` 链式持有根因,
//!     `serde` 直接序列化整条链;
...
//! - `PagodaError::{builder, msg, error_type, message}`;
//! - `PagodaErrorBuilder::{error_type, message, cause, build}`,默认 `Unknown` + 空消息 + 无根因;
//! - `From<&dyn Error>` / `From<Box<dyn Error>>`: 已是 `PagodaError` 走 clone/downcast,否则
...
impl PagodaError {
    /// 入口:返回一个新的 [`PagodaErrorBuilder`].
    pub fn builder() -> PagodaErrorBuilder {
        PagodaErrorBuilder::default()
    }
...
impl fmt::Display for PagodaError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
/// 从 `&dyn Error` 转换:已是 `PagodaError` 时克隆,否则递归包装.
impl<'a> From<&'a (dyn std::error::Error + 'static)> for PagodaError {
    fn from(err: &'a (dyn std::error::Error + 'static)) -> Self {
        ...
        if let Some(existing) = err.downcast_ref::<PagodaError>() {
            return existing.clone();
        }
        let caused_by = err.source().map(|inner| Box::new(PagodaError::from(inner)));
        PagodaError {
...
/// 从 `Box<dyn Error>` 转换:已是 `PagodaError` 时取出所有权,否则下放到引用版本.
impl From<Box<dyn std::error::Error + 'static>> for PagodaError {
    fn from(err: Box<dyn std::error::Error + 'static>) -> Self {
        match err.downcast::<PagodaError>() {
            Ok(boxed) => *boxed,
            Err(other) => PagodaError::from(&*other as &(dyn std::error::Error + 'static)),
        }
    }
}

// === SECTION: PagodaErrorBuilder ===
...
impl PagodaErrorBuilder {
    ...
    /// 设置根因.任意 `std::error::Error` 都被转换为 `PagodaError`.
    pub fn cause(mut self, cause: impl std::error::Error + 'static) -> Self {
        let converted = PagodaError::from(&cause as &(dyn std::error::Error + 'static));
        self.caused_by = Some(Box::new(converted));
        self
    }

    /// 完成构造.缺省值:`Unknown` / 空消息 / 无根因.
    pub fn build(self) -> PagodaError {
        PagodaError {
...
///
/// - 走 `source()` 链,只考察可 downcast 为 [`PagodaError`] 的节点;
...
mod tests {
    ...
    // 编译期断言：PagodaError 满足 std::error::Error + Send + Sync + 'static
    const _: () = {
    ...
    fn test_msg_constructor() {
        let err = PagodaError::msg("something failed");
    ...
    fn msg_accepts_string_and_str() {
        let a = PagodaError::msg("static str");
        let b = PagodaError::msg("owned string".to_string());
    ...
    #[test]
    fn builder_defaults() {
        let err = PagodaError::builder().build();
        assert_eq!(err.error_type(), ErrorType::Unknown);
    ...
    fn builder_all_fields() {
        let inner = PagodaError::msg("root cause");
        let err = PagodaError::builder()
    ...
    fn source_none_when_no_cause() {
        let err = PagodaError::msg("no cause");
        assert!(err.source().is_none());
    }
    ...
    fn three_level_source_chain() {
        let root = PagodaError::msg("root");
        let mid = PagodaError::builder().message("mid").cause(root).build();
        let top = PagodaError::builder().message("top").cause(mid).build();
    ...
    fn test_from_boxed_std_error() {
        ...
        let pagoda_err = PagodaError::from(boxed);
        assert_eq!(pagoda_err.error_type(), ErrorType::Unknown);
        assert_eq!(pagoda_err.message(), "io error");
    }

    /// Box<PagodaError> 转换时取出所有权，不额外包装
    #[test]
    fn test_from_boxed_takes_ownership_of_pagoda_error() {
        let inner = PagodaError::msg("original");
        let boxed: Box<dyn std::error::Error> = Box::new(inner);
        let pagoda_err = PagodaError::from(boxed);
        assert_eq!(pagoda_err.error_type(), ErrorType::Unknown);
        assert_eq!(pagoda_err.message(), "original");
    }
    ...
    fn test_from_boxed_with_source_chain() {
        ...
        assert_eq!(pagoda_err.message(), "outer error occurred");
        assert!(pagoda_err.source().is_some());
        assert!(pagoda_err.source().unwrap().to_string().contains("file not found"));
    }

    /// &dyn Error 转换：已是 PagodaError 时克隆保留类型
    #[test]
    fn from_ref_preserves_pagoda_error_type() {
        let orig = PagodaError::builder()
            .error_type(ErrorType::Cancelled)
            .message("cancelled")
            .build();
        let converted = PagodaError::from(&orig as &(dyn std::error::Error + 'static));
    ...
    fn test_serialization_roundtrip() {
        let cause = PagodaError::msg("inner cause");
        let err = PagodaError::builder()
            .error_type(ErrorType::Unknown)
            .message("outer error")
            .cause(cause)
        ...
        let deserialized: PagodaError = serde_json::from_str(&json).unwrap();
    ...
    fn serialization_omits_caused_by_when_none() {
        let err = PagodaError::msg("simple");
        let json = serde_json::to_string(&err).unwrap();
    ...
    fn match_error_chain_deep_hit() {
        let root = PagodaError::builder()
            .error_type(ErrorType::Disconnected)
            .message("root")
            .build();
        let outer = PagodaError::builder()
            .error_type(ErrorType::Unknown)
            .message("outer")
            .cause(root)
            .build();
    ...
    /// 非 PagodaError 节点被跳过
    #[test]
    fn match_error_chain_skips_non_pagoda_nodes() {
        // std::io::Error 不能 downcast 为 PagodaError，应被跳过
    ...
    fn match_error_chain_mid_match_bottom_exclude() {
        let bottom = PagodaError::builder()
            .error_type(ErrorType::Cancelled)
            .message("cancelled")
            .build();
        let mid = PagodaError::builder()
            .error_type(ErrorType::Disconnected)
            .message("disconnected")
            .cause(bottom)
            .build();
        let top = PagodaError::builder()
            .error_type(ErrorType::Unknown)
            .message("top")
            .cause(mid)
            .build();
