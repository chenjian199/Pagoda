...
//!
//! ## 设计意图
//! 在重放过程中累积每个请求的到达/准入/出 token 时间，最终汇总为延迟、吞吐与前缀复用率分布报告。
//!
//! ## 外部契约
//! 公开 `TraceSimulationReport` 及其 `Display`/`Serialize` 实现（JSON 字段名/数量与 Dynamo 一致），
//! 以及 `TraceDistributionStats` 等分布统计类型。
...
impl TraceCollector {
    ...
    /// 若可用，返回已完成请求的 (ttft_ms, mean_itl_ms)。
