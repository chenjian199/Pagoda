...
- **专业术语不翻译**：以下类别在注释中保留英文原文，无需强行汉化：
  - 技术名词：`token`、`hash`、`block`、`stream`、`cache`、`prefix`、`chunk`
  - 协议/格式：`JSON`、`EOF`、`API`、`HTTP`、`section`、`jail`
  - 算法/数据结构：`LRU`、`FIFO`、`Welford`、`trie`
  - 数学/统计：`variance`、`mean`、`count`
  - 编译/语言概念：`trait`、`struct`、`enum`、`impl`、`match`
  - 库/框架名：`tokio`、`serde`、`vLLM`、`async-openai`
  - 模型/厂商名：`OpenAI`、`Anthropic`、`Kimi K2`、`GPT`
  这些术语可以自然嵌入中文注释中，无需翻译（如"启用 EOF 恢复"、"截断的 JSON 参数"）。
...
```
此类注释中，`EOF`、`token`、`JSON`、`jail` 等术语无需翻译，直接嵌入中文语句即可。
...
- **必须**：SPDX 头占据文件首段，后面紧跟空行再开始模块文档注释
...
所有 Rust 源码（`.rs`）文件**必须**使用以下纯 Pagoda 两行头，
**不保留**双版权行或 NVIDIA 行：
...
#### 4b. `Cargo.toml` —— 保留 API 兼容实现说明

`Cargo.toml` 使用 `#` 前缀的 SPDX 头，并**必须保留** API 兼容实现说明段，
以标明该 crate 与上游 NVIDIA Dynamo 的接口兼容关系：
...
```
TOML/YAML/Python/Shell（非 Cargo.toml）:
...
```
1. 查看文件前 10 行
2. .rs 文件：确认仅为 PAGODA 两行头，无 NVIDIA 行，无 API-compatible 说明块
3. Cargo.toml：确认两行 SPDX + API-compatible 说明段完整保留
4. 确认 SPDX-FileCopyrightText 存在且至少有 PAGODA 行
5. 确认 SPDX-License-Identifier: Apache-2.0
6. 确认头部之后有空行分隔，再开始模块文档或 `[package]` 段
```
...
### 单个文件快速审核
```bash
# 步骤 1: 检查英文注释（找到后必须翻译，不得删除）
grep -E '[a-zA-Z]{3,}' <file> | grep -i '//'
...
# 步骤 4: 检查 SPDX（.rs 文件应为纯 PAGODA 两行头，无 NVIDIA 行）
head -5 <file> | grep 'SPDX'

# 步骤 4.1: 确认 .rs 文件头为正确两行格式
head -2 <file>

# 步骤 5: 对 Cargo.toml 确认保留了 API-compatible 说明段
head -10 Cargo.toml | grep 'API-compatible'
...
- 0 行未经翻译的英文 sentence 注释（所有英文注释均已翻译为等义中文）
- 0 个无标签的死代码块（>3 行）
- 0 个 `dyn-`/`dynamo-` 前缀
- 0 个 `comp`/`ep` 术语（应为 `sg`/`pn`）
- 所有 `.rs` 文件头为纯 Pagoda 两行 SPDX，无 NVIDIA 行
- 所有 `Cargo.toml` 保留 API-compatible 实现说明段
- 0 个 `lib-copy` 相关描述
