...
//! # `servicegroup::servicegroup` 子模块占位文件
//!
//! ## 设计意图
//!
//! 在早期版本中，`ServiceGroup` 类型上曾经直接 `impl EventPublisher` /
//! `impl EventSubscriber`，把"组件可以发布/订阅事件"作为 `ServiceGroup`
//! 本身的固有方法暴露给上层。这种做法把"组件命名 / 注册"与"事件平面"
//! 这两个原本应当解耦的关注点强绑在同一个类型上，会带来两个副作用：
//!
//! - 任何对事件协议的修改都必须穿过 `ServiceGroup` 类型本体
//! - 上层若想单独使用 `ServiceGroup`，会被迫拖入事件平面相关依赖
//!
//! 因此在迁移到统一事件平面（`transports::event_plane`）的过程中，这部
//! 分 `impl` 已经被移除。事件发布与订阅的能力改为通过
//! `EventPublisher::for_servicegroup(&servicegroup, topic)` 与
//! `EventSubscriber::for_servicegroup(&servicegroup, topic)` 这两个外部入口
...
//!    `portname.rs`、`registry.rs`、`service.rs`、`client.rs`）都以
//!    "模块名 + `.rs`"的形式组织，删除该文件会令模块布局看起来不一致。
//! 2. **记录历史决策** —— 后续读者在浏览源码时能在最直观的位置看到
//!    "事件平面的 `impl` 为何不在 `ServiceGroup` 上"的解释。
//! 3. **留出后续扩展点** —— 如果将来还需要在 `ServiceGroup` 类型本体上
