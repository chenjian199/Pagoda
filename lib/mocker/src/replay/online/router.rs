...
//!
//! ## 设计意图
//! 在实时重放中封装 KV 索引器与调度器，依据前缀重叠与负载为请求选择 worker，并跟踪 prefill/完成事件。
//!
//! ## 外部契约
//! 提供 `ReplayRouter`，通过 `on_request_arrival`/`on_first_token`/`on_request_completed` 等异步接口驱动路由，行为与 Dynamo 一致。
...
use pagoda_kv_router::ConcurrentRadixTree;
use pagoda_kv_router::config::KvRouterConfig;
use pagoda_kv_router::indexer::{
...
use pagoda_kv_router::scheduling::TierOverlapBlocks;
use tokio::sync::mpsc;
...
impl KvCacheEventSink for ReplayKvEventSink {
    fn publish(&self, event: pagoda_kv_router::protocols::KvCacheEvent) -> anyhow::Result<()> {
        self.event_tx
    ...
    fn publish_with_storage_tier(
        &self,
        event: pagoda_kv_router::protocols::KvCacheEvent,
...
impl KvReplayRouter {
    fn new(
        ...
        let scheduler = Arc::new(pagoda_kv_router::LocalScheduler::new(
            slots,
            worker_config_rx,
    ...
    ) -> Vec<pagoda_kv_router::PotentialLoad> {
