...
//!
//! ## 设计意图
//! 维护「等待/运行/被抢占」请求状态机，按 token 预算与并发上限调度 prefill/decode，
//! 与本地 KV 缓存交互分配/释放块，并在内存压力下按抢占策略驱逐请求。
//!
//! ## 外部契约
//! - 对 crate 内导出 `VllmCore` 及其构造与执行方法（`new`/`new_with_kv_capture`/
//!   `new_with_sink`/`receive`/`execute_pass`/`execute_hidden_pass`/`execute_pass_internal`）；
//!   对 super 暴露 `state`、`kv_cache` 字段供 live/tests 使用。
//! - 全部 panic/debug_assert 文案为可观察契约，保持原文不变。
//!
//! ## 实现要点
//! - `SchedulerState` 用队列 + 成员集合双结构维护，惰性清理已失效条目；
//! - 调度循环先处理 running，再从 waiting 准入，遇分配失败按策略抢占并回退预算；
//! - prefill/decode 时延由 perf_model 预测后按 speedup 比例缩放。
...
use crate::kv_cache::LocalVllmKvCache;
...
struct ScheduledWork {
    ...
    prompt_tokens: usize,
    prefix_tokens: usize,
    /// 完整 prompt 长度，在调度时捕获，用于 FPM 方差计算。
    prompt_len: usize,
    /// 调度时的总序列长度（prompt + 已生成），用于 FPM 的 decode KV 上下文。
    /// 在此捕获是因为已完成请求会在 `compute_fpm` 运行前从 state 中移除。
    sequence_len: usize,
}
...
impl SchedulerState {
    ...
    /// 从等待队列队首移除 `uuid` 并从 `waiting_members` 集合移除。
    /// 供 `transition_to_running`（随后晋升为 running）复用。
...
pub(crate) struct VllmCore {
    ...
    pub(super) kv_cache: LocalVllmKvCache,
...
impl VllmCore {
    ...
    fn new_internal(
        ...
        Self {
            kv_cache: LocalVllmKvCache::new_with_event_sink(
                args.num_gpu_blocks,
                args.block_size,
                kv_event_publishers,
                dp_rank,
    ...
    pub(super) fn execute_pass_internal(
        ...
        EnginePassResult {
            ...
            output_signals,
            admissions,
            active_decode_blocks: self.kv_cache.num_active_blocks() as u64,
    ...
    /// 从刚结束的一轮计算前向度量快照。
    ///
    /// `scheduled` 含本轮被调度的工作项。每请求元数据（prompt_len、sequence_len）在调度时
    /// 已记入 `ScheduledWork`，故本方法对已调度请求不依赖 `self.state.requests`
    /// （已完成请求可能已被移除）。队列度量取调用时刻的 `self.state.waiting`。
    fn compute_fpm(
        &self,
    ...
    fn schedule_request(
        ...
        loop {
            ...
            let allocated = self.kv_cache.process(&signal);
    ...
    fn emit_ready_tokens(
        ...
        // 对 prefill worker，第一个 decode token 在 prefill 前向过程中产出——无需单独的 decode 迭代。
        let (decode_time, decode_end_ms) = if self.args.worker_type == WorkerType::Prefill {
            (Duration::ZERO, decode_start_ms)
        } else {
            let active_kv_tokens = self.kv_cache.num_active_blocks() * self.args.block_size;
        ...
        for uuid in ready {
            ...
            loop {
                ...
                if process_signals(&mut self.kv_cache, &signals) {
...
fn process_signals(kv_cache: &mut LocalVllmKvCache, signals: &[MoveBlock]) -> bool {
    for signal in signals {
        if kv_cache.process(signal) > 0 {
            continue;
        }
