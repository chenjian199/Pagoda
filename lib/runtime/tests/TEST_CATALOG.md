...
本文档对 `lib/runtime/tests/` 下各测试文件及用例进行说明，便于理解覆盖范围、生产代码对应关系与运行方式。内容与仓库内 `integration_*.rs` 源码注释同步维护。

## 目录结构概览

| 类别 | 文件 | 运行方式 | 外部依赖 |
|------|------|----------|----------|
| 集成契约测试 | `integration_*.rs`| PR 必跑（部分 `#[ignore]`） | 多数无；etcd/NATS/K8s 见各文件 |
...
| 文件 | PR | `#[ignore]` | 合计 |
|------|-----|-------------|------|
| `integration_runtime_lifecycle` | 5 | 0 | 5 |
| `integration_component_routing` | 9 | 0 | 9 |
| `integration_request_plane`（tcp + nats） | 6 | 7 | 13 |
| `integration_event_plane`（zmq + nats） | 2 | 1 | 3 |
| `integration_pipeline`（含 `disaggregated`） | 5 | 0 | 5 |
| `integration_discovery`（file + etcd + kube） | 6 | 11 | 17 |
| `integration_storage` | 3 | 2 | 5 |
| `integration_health_metrics` | 6 | 0 | 6 |
| `integration_system_status`（含 engine/local） | 8 | 0 | 8 |
| `integration_compute` | 2 | 0 | 2 |
| `integration_config_env` | 5 | 0 | 5 |
| `integration_failures` | 5 | 0 | 5 |
| `integration_soak` | 1 | 4 | 5 |
| **合计** | **63** | **25** | **88** |
...
cargo test -p dynamo-runtime --tests -- --test-threads=1

# 单个集成测试文件
cargo test -p dynamo-runtime --test integration_discovery -- --test-threads=1

# Nightly：etcd discovery
ETCD_ENDPOINTS=http://127.0.0.1:2379 \
  cargo test -p dynamo-runtime --test integration_discovery \
  --features testing-etcd -- --test-threads=1 --include-ignored

# Release：K8s discovery（需集群 + DynamoWorkerMetadata CRD）
cargo test -p dynamo-runtime --test integration_discovery \
  --features integration-kube -- --test-threads=1 --include-ignored

# Nightly：NATS request plane + event plane
NATS_SERVER=nats://127.0.0.1:4222 \
  cargo test -p dynamo-runtime --test integration_request_plane \
  --test integration_event_plane -- --test-threads=1 --include-ignored
```
...
### `integration_runtime_lifecycle.rs`
...
| `shutdown_rejects_new_requests_after_draining_inflight` | graceful shutdown 排空 in-flight 后拒绝新请求 | `runtime.rs` + `endpoint.rs`：Phase 1 注销 endpoint；等待 inflight；Phase 3 取消主 token | in-flight 收到 `drained`；第二次 `generate` 返回 no instances 类错误 |

---

### `integration_component_routing.rs`

验证 `namespace → component → endpoint` 路由隔离、discovery 注册/注销与副本池行为。

| 用例 | 目的/场景 | 生产逻辑 | 关键断言 |
|------|-----------|----------|----------|
| `register_discover_call_unregister_roundtrip` | 注册 → discovery → RPC → 注销完整闭环 | `Endpoint::start` 注册；`unregister_endpoint_instance` 调用 `discovery.unregister` | 注销前流式响应 `a,b,c`；注销后 RPC 失败 |
...
| `same_endpoint_same_namespace_forms_replica_pool` | 同逻辑 endpoint 多 worker 形成副本池 | file KV 共享 discovery；`PushRouter::round_robin` 轮询 | discovery 2 实例；RPC tag 至少 2 种 |
| `same_namespace_different_component_is_isolated` | component 为路由隔离维度 | discovery key 含 component | `component-b` 无实例 |
| `same_namespace_same_component_different_endpoint_is_isolated` | endpoint 名为路由隔离维度 | 同 component 下不同 endpoint 独立池 | `generate-b` 无实例 |
| `calling_unregistered_endpoint_returns_unavailable` | 从未注册的 endpoint 快速失败 | `PushRouter` 在 `instance_ids_avail().is_empty()` 时报错 | `generate_expect_no_instances` |
| `calling_endpoint_after_unregister_returns_unavailable` | 注销后调用失败（区别于从未注册） | unregister → discovery Removed → watch 收敛 | 注销前成功；注销后失败 |
| `duplicate_instance_registration_is_idempotent_or_rejected_consistently` | 重复 `register_endpoint_instance` 行为稳定 | KV `insert` 同 key 覆盖 | `client.instances().len() == 1` |
| `different_models_same_logical_endpoint_are_rejected` | 同一逻辑 endpoint 不允许冲突 model 名 | `Discovery::register` 检查 `find_conflicting_model_name` | 第二次 register Err 含 `Cannot register model 'model-b'` |

---

### `integration_request_plane.rs`
...
| `nats_subject_includes_routing_identity` | NATS subject 含路由身份 | `build_transport_type` → `nats::instance_subject` | subject 含 namespace/component/endpoint/instance id |
...
| `nats_request_timeout_returns_error` | handler 长时间不产出时客户端超时 | `DYN_HTTP_BACKEND_STREAM_TIMEOUT_SECS` | 流含 timeout/error 项 |
...
| `nats_service_registers_component_endpoints` | NATS microservice 注册 component 多 endpoint | `register_nats_service` + `NatsMultiplexedServer`；`$SRV.STATS` | ≥2 endpoint；name 含 generate/health |
| `nats_service_subject_matches_routing_identity` | service endpoint subject 含路由身份 | slugify `{ns}_{component}` + `{endpoint}-{instance_hex}` | subject/name 含 service、endpoint、instance id |

---

### `integration_event_plane.rs`
...
| 用例 | 目的/场景 | 生产逻辑 | 关键断言 |
|------|-----------|----------|----------|
| `file_discovery_watch_sees_register_and_unregister` | file KV watch 观察 Added/Removed | `KVStoreDiscovery::list_and_watch` | Added instance_id 一致；Removed 匹配 endpoint |
...
| `file_discovery_filters_exact_namespace_component_endpoint` | query 精确匹配三元组 | `KVStoreDiscovery::query_prefix` | 精确 1 条；错 endpoint/namespace 为空 |
...
| `kube_discovery_filters_exact_namespace_component_endpoint` | K8s query 精确过滤 | `MetadataSnapshot::filter` | 精确 query 1 条；错 endpoint/namespace `wait_for_discovery_list` 为 0 |
...
| `kube_container_mode_register_list_roundtrip` | container 模式 register/list | `DYN_KUBE_DISCOVERY_MODE=container`；Pod `containerStatuses.ready` | list 1 条；`instance_id == hash_pod_name(pod)` |
...
### `integration_health_metrics.rs`

验证 endpoint/system health 与 Prometheus metrics 导出。

| 用例 | 目的/场景 | 生产逻辑 | 关键断言 |
|------|-----------|----------|----------|
| `health_check_target_registration_notifies_manager` | 注册 health check target 时通知 `HealthCheckManager` | `register_health_check_target` + `new_endpoint_tx`（`DYN_HEALTH_CHECK_ENABLED`） | `get_health_check_target` / `get_endpoint_health_check_notifier` 均存在 |
| `endpoint_health_tracks_registration` | endpoint 启动后 system health 记录 Ready | `Endpoint::start` → `set_endpoint_registered` | `get_endpoint_health_status == Ready` |
| `endpoint_health_tracks_canary_result` | canary 成功后 health 变 Ready | `HealthCheckManager` + local registry | 初始 NotReady；5s 内变 Ready |
| `system_health_uses_endpoint_status_when_configured` | 配置后系统健康由 endpoint 聚合 | `DYN_SYSTEM_USE_ENDPOINT_HEALTH_STATUS` | 注册前 unhealthy；注册后 healthy 且含 `generate: ready` |
| `metrics_labels_include_runtime_identity` | metrics 含 endpoint 层级标签 | `WorkHandlerMetrics::from_endpoint` | scrape 含 component、endpoint |
| `metrics_names_are_prometheus_safe` | 指标名与 label 符合 Prometheus 规范 | `metrics/prometheus_names.rs` sanitize | 内置名与 scrape 每行均通过校验 |

---

### `integration_system_status.rs`
...
| `health_endpoint_reports_aggregated_health` | `/health` 随 endpoint 注册变化 | `health_handler` + `SystemHealth` | 注册前 503 notready；注册后 200 ready |
| `status_endpoint_includes_registered_endpoints` | health JSON 列出已注册 endpoint | `endpoints` 字段来自 `SystemHealth` | `endpoints.generate == "ready"` |
| `custom_health_and_live_paths_are_honored` | 自定义 health/live 路径生效 | `DYN_SYSTEM_*_PATH` env | 默认路径 404；自定义路径 200 |
...
| 用例 | 目的/场景 | 生产逻辑 | 关键断言 |
|------|-----------|----------|----------|
| `local_endpoint_registry_lists_registered_engines` | local registry register/get 与 engine routes 列表 | `LocalEndpointRegistry` + `EngineRouteRegistry::routes` | 已注册 get 为 Some；routes 含两条路径 |

---

### `integration_compute.rs`
...
> Process harness 经 `Runtime::from_settings` + `DYN_COMPUTE_THREADS=2` 初始化 compute pool。

---

### `integration_config_env.rs`

验证运行时配置从环境变量读取及测试隔离。

| 用例 | 目的/场景 | 生产逻辑 | 关键断言 |
|------|-----------|----------|----------|
| `runtime_config_reads_env_over_defaults` | env 覆盖默认配置 | `DistributedConfig::from_settings`；`get_tcp_rpc_host_from_env`；`DYN_DISCOVERY_BACKEND=mem` | `request_plane == Http`；TCP host 匹配 env |
...
| `network_advertise_host_uses_env_when_set` | HTTP/TCP advertise host 优先 env | `get_http_rpc_host_from_env` / `get_tcp_rpc_host_from_env` | 返回值与 `DYN_HTTP_RPC_HOST` / `DYN_TCP_RPC_HOST` 一致 |
...
| `external_discovery_outage_returns_unavailable_then_recovers` | 外部 discovery 短暂不可用后恢复 | `wipe_file_discovery_namespace` 模拟 outage；`register_endpoint_instance` 恢复 | outage 期间 no instances；恢复后 echo 成功 |
...
| 用例 | 目的/场景 | 运行 | 关键断言 |
|------|-----------|------|----------|
| `soak_smoke_completes_short_run` | 2s 冒烟：持续 RPC 稳定完成 | PR 必跑；`DYN_SOAK_RUN_DURATION=2s`，batch=8 | backend 计数 > 0；无 panic |
| `soak_sustained_load_reports_diagnostics` | 长 soak（默认 30s，batch 64） | `#[ignore]` + `--include-ignored` | 完整 duration 内 RPC 无失败 |
| `random_endpoint_churn_does_not_leave_stale_instances` | 6 endpoint 伪随机 register/unregister churn | `#[ignore]` + `--include-ignored` | `discovery.list` 与 `client.instances()` 一致；file KV 0 残留 |
...
| 用例 | 说明 |
|------|------|
| `integration::main` | 读 `DYN_SOAK_RUN_DURATION`、`DYN_SOAK_BATCH_LOAD`、`DYN_QUEUED_UP_PROCESSING`；并发 batch RPC 测吞吐 |
...
| 模块 | 职责 |
|------|------|
| `contract.rs` | 集成测试主 harness：`process_local_runtime`、`shared_integration_runtime`、`file_backed_runtime`、`etcd_runtime`、`nats_*`、`serve_endpoint_*`、`shutdown_runtime`、`acquire_contract_test_lock`、`unique_name` 等 |
...
| Feature / 环境变量 | 影响的用例 |
|--------------------|------------|
| （无，默认） | 绝大多数 `integration_*` 顶层与 `mod file` / `mod tcp` / `mod zmq` / `mod memory` 用例 |
| `testing-etcd` + `ETCD_ENDPOINTS` + `--include-ignored` | `integration_discovery::etcd::*`（3）；`integration_storage::etcd::*`（1） |
| `integration-kube` + K8s + `DynamoWorkerMetadata` CRD + `--include-ignored` | `integration_discovery::kube::*`（8） |
| `NATS_SERVER` + `--include-ignored` | `integration_request_plane::nats::*`（7）；`integration_storage::nats::*`（1）；`integration_event_plane::nats::*`（1） |
| `--include-ignored`（Release soak） | `integration_soak::{soak_sustained_load_*,random_endpoint_churn_*,runtime_shutdown_after_soak_*,long_running_streams_*}`；`DYN_SOAK_*` 仅影响 sustained load |
