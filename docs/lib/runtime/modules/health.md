...
模块职责：管理每个 PortName 的健康检查任务生命周期，发送探测请求，更新健康状态。
...
**为什么用 `portname_subject` 而非 `PortName` 对象做 Key**：`PortName` 是富类型（含 DRT 引用），不适合作为 HashMap Key（需要实现 Hash 等）。`portname_subject` 是 `String`，直接使用 `HashMap<String, ...>` 即可。

**`Arc<Mutex<...>>`**：`RouterCache` 在多个异步任务间共享（每个 PortName 有独立的健康检查任务），且访问发生在 async 上下文中（路由器创建涉及 await），必须使用 async 锁。`parking_lot::Mutex`（同步锁）仅适用于持锁时无 await 的场景。
...
健康检查是一个持续运行的后台系统，涉及多个并发任务（每个 PortName 一个任务）的生命周期管理。将状态集中在 `HealthCheckManager` 中，调用方只需创建一个 `Arc<HealthCheckManager>` 并调用 `start()`，内部的任务管理、缓存维护、状态更新都由 Manager 负责。

**`drt: DistributedRuntime`**：访问 `SystemHealth`（获取注册的探测目标、更新健康状态）和 `Namespace/ServiceGroup/PortName` 对象（构造发送探测请求所需的路由路径）。DRT 是廉价克隆的，直接持有一份克隆。
...
let mut rx = manager
    ...
    .ok_or_else(|| anyhow::anyhow!("PortName receiver already taken"))?;
```
...
从 DRT 出发，按命名层级构建 `PortName` 对象。此 PortName 对象被传给 `get_or_create_router` 用于创建或查找 PushRouter。
