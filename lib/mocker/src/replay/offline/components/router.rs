...
//!
//! ## 设计意图
//! 在虚拟重放时间轴上复现 KV Router 的前缀缓存索引、worker 选择与排队阈值决策，驱动请求准入与排队。
//!
//! ## 外部契约
//! 公开 `OfflineReplayRouter`，方法 `on_request_arrival`/`on_kv_events`/`add_worker`/`remove_worker` 的返回值与错误文案与 Dynamo KV Router 保持一致。
...
use pagoda_kv_router::queue::DEFAULT_MAX_BATCHED_TOKENS;
...
/// 一次成功 ``admit_request`` 调用的内部结果：选中的
/// worker 加上 router 看到的前缀缓存重叠情况，以便调用方
/// 将重叠统计转发给流量累加器。
...
impl OfflineReplayRouter {
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            // 这仅是一个基准 Instant，用于将重放的 `now_ms` 值转换为
            // 合成的 `Instant`。后续所有衰减/记账都使用基于该基准点派生的
            // 虚拟重放时间，而非墙钟推进。
    ...
    /// 向 router 注册一个新 worker，不扰动现有的 slot 状态。
    ...
    /// 将一个 worker 从路由资格中移除。
    ///
    /// 仅从配置映射中移除该 worker，使选择器不会为新请求
    /// 选中它。radix tree 与活动序列 slot 保持不变，以便该 worker
    /// 上的在飞请求仍可完成（free / mark_prefill_completed），且 KV
    /// 事件仍可引用现有块而不会报 "parent block not found" 错误。
    /// 陈旧的 slot 与索引器状态是无害的 —— 选择器与
    /// `all_workers_busy` 都会跳过不在 `workers_with_configs` 中的 worker。
...
mod tests {
    ...
    use pagoda_kv_router::PrefillLoadEstimator;
    use pagoda_kv_router::config::{KvRouterConfig, RouterPrefillLoadModel};
    use pagoda_kv_router::protocols::{
