...
//!
//! ## 设计意图
//! 实现离线（聚合/分离/单 worker）trace 与并发重放的核心入口，构建运行时、驱动事件循环并汇总报告。
//!
//! ## 外部契约
//! 提供 `simulate_trace`/`simulate_concurrency`/`generate_trace_worker_artifacts` 等 pub(crate) 入口，行为与 Dynamo 保持一致。
