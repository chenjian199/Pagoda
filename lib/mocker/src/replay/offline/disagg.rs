...
//!
//! ## 设计意图
//! 实现分离（prefill 与 decode 位于不同 worker）的离线离散事件仿真运行时，驱动两阶段的准入、路由、交接与事件推进，并支持与 planner 的步进式集成。
//!
//! ## 外部契约
//! 提供 `DisaggRuntime` 及其 `new`/`new_workload`/`run`/`advance_to` 等接口，返回 `TraceCollector`/统计，行为与 Dynamo 一致。
...
pub(in crate::replay) struct DisaggRuntime {
    ...
    /// 在相邻 planner tick 之间累积的前向传播指标，以 (stage, worker_idx) 为键。
...
impl DisaggRuntime {
    ...
    /// 统计跨 prefill、decode 与路由器队列占用集群容量的所有请求。
    ...
    /// 将请求的 prefill 阶段分发到指定的 prefill worker。
    ...
    /// 将请求的 decode 阶段分发到指定的 decode worker。
    ...
    /// 将 prefill 路由器的准入结果转化为具体的 worker 分发。
    ...
    /// 将 decode 路由器的准入结果转化为具体的 worker 分发。
    ///
    /// 注意：仅 prefill 路由器的准入被馈入
    /// ``traffic.on_admission``；decode 路由器的准入反映的是
    /// 同一批请求在 prefill 完成后重新路由，若计入会对
    /// overlap 观测重复计数。
    ...
    /// 根据 decode 路由器是否活跃，将请求排队或直接分发到 decode。
    ...
    /// 将一个外部请求准入到 prefill 侧状态、采集器状态与可选路由器。
    ...
    /// 从到达、worker 完成或 decode 交接中选取下一个逻辑时间戳。
    ...
    /// 在调度器选定的可见性阶段应用 prefill 侧的 KV 路由器事件。
    ...
    /// 处理一个 prefill 输出信号，包括路由器更新与 decode 交接调度。
    ...
    /// 处理一个 decode 输出信号，包括 decode 路由器释放与请求完成。
    ...
    /// 要么立即入队 decode，要么在事件堆上调度一个延迟交接事件。
    ...
    /// 在当前时间戳启动每个可推进的空闲 prefill worker 的 pass。
    ...
    /// 在当前时间戳启动每个可推进的空闲 decode worker 的 pass。
    ...
    /// 应用一个分别针对 prefill 与 decode 目标的扩缩决策。
    ///
    /// 扩容：若相应引擎参数上配置了 `startup_time`，
    /// 新 worker 进入启动阶段并调度一个 `WorkerReady` 事件。
    /// 只有该事件触发时它们才变为活跃（并注册到
    /// 路由器）。若无 `startup_time`，worker 立即可用。
    ///
    /// 缩容：worker 被立即从路由器移除，使新请求不再
    /// 落在其上，同时它排空 in-flight 工作。
    ...
    /// 运行分阶段离线重放，直到 prefill 与 decode 两条流水线均被排空。
...
fn derive_decode_router_config(
    ...
    config.router_prefill_load_model = pagoda_kv_router::config::RouterPrefillLoadModel::None;
    config
}
