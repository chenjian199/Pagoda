...
max_message_size: usize
  当前连接允许接收的最大 frame
  默认来自 DYN_TCP_MAX_MESSAGE_SIZE，否则 32MB
```
...
HTTP:
  {DYN_HTTP_RPC_ROOT_PATH}/{portname_name}
...
```text
1. 读取 DYN_TCP_WORKER_POOL_SIZE，默认 1500
2. 读取 DYN_TCP_WORK_QUEUE_SIZE，默认 6000
...
这样避免了多个 worker task 争抢同一个 receiver 时的额外锁竞争，同时仍然把并发度限制在 `DYN_TCP_WORKER_POOL_SIZE`。
...
```text
{DYN_HTTP_RPC_ROOT_PATH}/{portname}
```
...
```text
max_frame_size:
  DYN_HTTP2_MAX_FRAME_SIZE，默认 1MB

max_concurrent_streams:
  DYN_HTTP2_MAX_CONCURRENT_STREAMS，默认 1000

pool_max_idle_per_host:
  DYN_HTTP2_POOL_MAX_IDLE_PER_HOST，默认 100

pool_idle_timeout:
  DYN_HTTP2_POOL_IDLE_TIMEOUT_SECS，默认 90s

keep_alive_interval:
  DYN_HTTP2_KEEP_ALIVE_INTERVAL_SECS，默认 30s

keep_alive_timeout:
  DYN_HTTP2_KEEP_ALIVE_TIMEOUT_SECS，默认 10s

adaptive_window:
  DYN_HTTP2_ADAPTIVE_WINDOW，默认 true

request_timeout:
  DYN_HTTP_REQUEST_TIMEOUT，默认 5s
```
...
```text
request_timeout:
  DYN_TCP_REQUEST_TIMEOUT，默认 5s

pool_size:
  DYN_TCP_POOL_SIZE，默认每个地址 100 条连接

connect_timeout:
  DYN_TCP_CONNECT_TIMEOUT，默认 5s

channel_buffer:
  DYN_TCP_CHANNEL_BUFFER，默认每条连接 writer channel 50
```
