...
**检查清单**：
```
1. grep -E '[a-zA-Z]{3,}.*//|/\*.*[a-zA-Z]{3,}' <file> 找可疑行
2. 排除以下不算英文：
   - URL (http://, https://, file://)
...
- **禁止**：`Dynamo`、`NVIDIA CORPORATION` 版权声明
- **必须**：将其描述为 Pagoda 独立项目

**检查清单**：
```
1. grep -E 'dyn-|dynamo-|comp[^a-z]|ep[^a-z]|\{.*comp.*\}|lib-copy|NVIDIA' <file>
2. 每行判断是否需要替换
3. 常见替换对：
   - s/dynamo-/pagoda-/g
   - s/dyn([^a-z])/pgd\1/g
...
**规则定义**：
- **必须**：所有新建源码文件都有 Pagoda SPDX 头
- **必须**：`Copyright (c) 2026-2028 PAGODA CORPORATION & AFFILIATES. All rights reserved.`
- **必须**：`SPDX-License-Identifier: Apache-2.0`
- **必须**：SPDX 头是文件首段非空内容
- **禁止**：过期的 `NVIDIA CORPORATION`、`2024-2026` 日期范围
...
# 步骤 3: 检查旧术语
grep -E 'dyn|comp|ep|lib-copy|NVIDIA' <file>
