...
//!
//! ## 设计意图
//! 为调度器与 KV 事件测试提供可复用脚手架：封装 LocalKvIndexer、抓取型 sink、
//! FPM 快照抓取以及“请求全部完成”的端到端断言。
//!
//! ## 外部契约
//! crate 内部测试工具，非公开 API；保留断言文案与错误字符串不变。
...
/// 向调度器发送 `num_requests` 个请求，收集全部输出信号，并断言调度器
/// 息产出恰好 `num_requests * max_output_tokens` 个信号且回到空闲（0 个活跃 decode 块）。
///
/// 当 `use_shared_tokens` 为 true 时，每个请求的前半部分共享公共前缀，
/// 用于验证前缀缓存 / radix 树复用。
