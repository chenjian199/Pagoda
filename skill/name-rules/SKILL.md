...
4. 检查服务发现命名是否符合 `Namespace / ServiceGroup / PortName`。
5. 检查模型实例是否保留 `topo_json: serde_json::Value`。
6. 检查 NVTX 相关对外命名是否迁移为 timeline 语义。
7. 运行搜索确认没有不应保留的旧命名残留。
