...
//!
//! ## 设计意图
//! 提供 trace 文件/请求的离线、在线、聚合、分离等多种重放入口，统一参数校验、trace 加载与归一化后转交底层 simulate 实现。
//!
//! ## 外部契约
//! 公开 `simulate_trace_*`/`simulate_concurrency_*`/`generate_trace_worker_artifacts_offline` 等函数，
//! 签名、错误文案（如 applied_compute_agentic 限制）与 Dynamo 保持一致。
...
fn single_turn_mooncake_requests(
    ...
    if trace_format == TraceFileFormat::Mooncake && trace.is_single_turn() {
        // 带时间戳的请求路径要求每个请求都携带到达时间戳；
        // 若没有这道保护，缺失 `first_arrival_timestamp_ms` 的 trace
        // 会在 `normalize_trace_requests` 中 panic，
        // 而不是返回清晰的错误。
