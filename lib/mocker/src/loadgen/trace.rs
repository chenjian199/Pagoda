...
//!
//! ## 设计意图
//! 从 Mooncake / AppliedComputeAgentic 轨迹文件解析会话轨迹，并提供合成轨迹、
//! token 合成、重放哈希计算与路由序列划分等能力。
//!
//! ## 外部契约
//! `TurnTrace`/`Trace` 的公开方法签名、serde 字段名与 alias、错误文案（如
//! "trace_block_size must be greater than 0"、"engine_block_size must be greater than 0"等）
//! 与 Dynamo 完全一致。
...
use pagoda_tokens::compute_hash_v2;
...
impl Trace {
    pub fn from_mooncake(path: &Path, trace_block_size: usize) -> Result<Self> {
        ...
        for (line_idx, line) in reader.lines().enumerate() {
            ...
            // 将 input_length 限制到可合成容量：在 mooncake 轨迹格式中，
            // input_length 为完整 prompt token 数，可能超过 hash_ids.len() * block_size（仅缓存部分）。
