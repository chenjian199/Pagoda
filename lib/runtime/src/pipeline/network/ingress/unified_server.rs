...
//! 的统一抽象，使上层 component / discovery 代码在切换传输时不需修改业务逻辑。
//! 设计原则：transport-agnostic、多路复用单端口、默认 async、提供健康查询。
//!
//! ## 外部契约
//! - `trait RequestPlaneServer: Send + Sync`，提供五个方法：
//!   - `register_endpoint(endpoint_name, service_handler, instance_id,
//!     namespace, component_name, system_health)`: 注册一个处理进入请求的 handler；
//!     `system_health` 为 `Arc<parking_lot::Mutex<SystemHealth>>` 原始类型。
//!   - `unregister_endpoint(endpoint_name)`: 取消注册。
...
mod tests {
    ...
    //! | `test_fake_server_unregister_unknown_endpoint_is_noop` | 取消注册不存在端点应 Ok |
    ...
    impl RequestPlaneServer for FakeServer {
        async fn register_endpoint(
            ...
            if !g.insert(endpoint_name.clone()) {
                anyhow::bail!("endpoint '{endpoint_name}' already registered");
            }
            Ok(())
        }
    ...
    // 最小化的 PushWorkHandler 占位，仅用于满足 register_endpoint 的签名。
    struct NoopHandler;
    #[async_trait]
    ...
    async fn test_fake_server_register_unregister_round_trip() {
        ...
        s.unregister_endpoint("ep1").await.unwrap();
        assert!(!s.registered.lock().await.contains("ep1"));
    }
    ...
    #[tokio::test]
    async fn test_fake_server_unregister_unknown_endpoint_is_noop() {
        let s = FakeServer::new();
        // 不存在的端点也应 Ok
        s.unregister_endpoint("never-registered").await.unwrap();
    }
    ...
    async fn test_fake_server_register_duplicate_returns_error() {
        ...
        .unwrap();
        let err = s
            .register_endpoint("dup".into(), h, 2, "ns".into(), "comp".into(), make_health())
            .await
