...
//!
//! ## 设计意图
//! 汇聚离线/在线重放子模块，定义 `ReplayRouterMode`/`ReplayArgsMode` 等共享枚举，并提供 trace 请求归一化入口。
//!
//! ## 外部契约
//! 重导出 `simulate_*` 系列入口函数、`OfflineDisaggReplayConfig`、`TraceSimulationReport` 等公开类型；
//! `normalize_trace_requests` 的排序、错误文案与 Dynamo 保持一致。
