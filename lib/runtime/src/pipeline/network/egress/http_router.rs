...
/// HTTP 请求的默认超时时间（只等 ACK，不等完整响应）。
const DEFAULT_HTTP_REQUEST_TIMEOUT_SECS: u64 = 5;

/// HTTP/2 性能配置常量。
const DEFAULT_MAX_FRAME_SIZE: u32 = 1024 * 1024; // 1MB 帧大小，提升吞吐。
const DEFAULT_MAX_CONCURRENT_STREAMS: u32 = 1000; // 允许更多并发流。
const DEFAULT_POOL_MAX_IDLE_PER_HOST: usize = 100; // 增大连接池。
const DEFAULT_POOL_IDLE_TIMEOUT_SECS: u64 = 90; // 让连接保持更久。
const DEFAULT_HTTP2_KEEP_ALIVE_INTERVAL_SECS: u64 = 30; // 每 30 秒发送一次 ping。
const DEFAULT_HTTP2_KEEP_ALIVE_TIMEOUT_SECS: u64 = 10; // ping 响应超时。
const DEFAULT_HTTP2_ADAPTIVE_WINDOW: bool = true; // 启用自适应流控。

// ─────────────────────────────────────────────────────────────────────────────
// === SECTION: [2] 私有 helper（env 解析 / 错误构造） ===
// ─────────────────────────────────────────────────────────────────────────────

/// 读取环境变量并调用 `T::from_str` 解析；变量未设置或解析失败时都返回 `None`。
///
/// 这个 helper 把 `Http2Config::from_env` 中 8 次重复的 `env::var().ok().and_then(|s| s.parse())`
/// 收敛成一行调用。
fn env_parse<T: FromStr>(name: &str) -> Option<T> {
    std::env::var(name).ok().and_then(|v| v.parse::<T>().ok())
}

/// 把 `reqwest::Error` 包装成 `anyhow::Error(PagodaError::CannotConnect)`，并保留 cause 链。
...
impl HttpRequestClient {
    /// 使用 HTTP/2 和默认配置创建一个新的 HTTP 请求客户端。
    pub fn new() -> Result<Self> {
        Self::with_config(Http2Config::default())
    }

    /// 使用自定义超时创建一个新的 HTTP 请求客户端（旧方法）。
    /// 使用 HTTP/2 prior knowledge 来避免 ALPN 协商开销。
    ...
    /// 使用基础配置创建一个新的 HTTP 请求客户端。
    ///
    /// 注意：较高级的 HTTP/2 配置方法并非所有 reqwest 版本都支持。
    /// 这个实现只使用稳定且广泛支持的配置选项。
...
mod tests {
    ...
    //! | 测试名 | 覆盖维度 |
    //! |---|---|
    //! | `test_http_client_creation` | 构造默认客户端成功 |
    //! | `test_http_client_with_custom_timeout` | `with_timeout` 覆盖 `request_timeout` |
    //! | `test_http2_config_from_env` | 多变量覆盖路径 |
    //! | `test_http_client_with_custom_config` | 所有字段透传到 client |
    //! | `test_http_client_send_request_invalid_url` | 无法连接 → Err |
    //! | `test_http2_client_server_integration` | 端到端 axum 集成 |
    //! | `test_http2_headers_propagation` | 自定义 header 透传 |
    //! | `test_http2_concurrent_requests` | HTTP/2 多路并发 |
    //! | `test_http2_performance_benchmark` | 性能基线 |
    ...
    async fn test_http2_client_server_integration() {
        ...
        // 创建使用 prior knowledge 的 HTTP/2 客户端。
        let client = HttpRequestClient::new().unwrap();
